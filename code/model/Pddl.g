...
//prog: ( domain {print $domain.tree.toStringTree();} )+ ;


getDomain: ( domain {return $domain.tree;} )+ ;
getProblem: ( problem {return $problem.tree;} )+ ;
...
actionDef
	: '(' ':action' actionSymbol
           actionDefBody ')'
       -> ^(ACTION actionSymbol actionDefBody)
    ;
...
actionDefBody
	: ( ':parameters' '(' typedVariableList ')')?
	  ...
	  ( ':effect' (('(' ')') | effect))?
	  -> ^(PARAMETERS typedVariableList?) ^(PRECONDITION goalDesc?) ^(EFFECT effect?)
	;
