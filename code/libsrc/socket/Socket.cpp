...
Socket::Socket (void)
{
	...
	pthread_mutex_init (&mtx_SendBuffer, NULL);
...
Socket::~Socket (void)
{
	...
	pthread_mutex_destroy (&mtx_SendBuffer);
}
...
long Socket::SendBlocking (const void* _zData, long _lBytes)
{
	...
	if (-1 == lSentBytes)
	{
		// we might get a -1 because the peer is not ready	
		// to receive more data (i.e., there's too much 	
		// data in the pipe already...						
...
bool Socket::SendNonBlocking (const void* _zData, long _lBytes)
{
	...
	pthread_mutex_lock (&mtx_SendBuffer);
	o_SendBuffer.Append (_zData, _lBytes);
	pthread_mutex_unlock (&mtx_SendBuffer);
