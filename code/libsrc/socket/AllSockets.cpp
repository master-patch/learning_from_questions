...
InternalRequest_dq_t	AllSockets::dq_InternalSocketManagementRequest;
ManagementRequest_dq_t	AllSockets::dq_ExternalSocketManagementRequest;
pthread_mutex_t			AllSockets::mtx_ExternalSocketManagement = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t			AllSockets::mtx_AllSockets = PTHREAD_MUTEX_INITIALIZER;


//															
bool AllSockets::IsValidSocket (Socket* _pSocket)
{
	return (set_AllSockets.end () != set_AllSockets.find (_pSocket));
}
...
void AllSockets::Add (Socket* _pSocket)
{
	...
	pthread_mutex_lock (&mtx_AllSockets);
	dq_InternalSocketManagementRequest.push_back (InternalSocketRequest (ADD_SOCKET, _pSocket));
	pthread_mutex_unlock (&mtx_AllSockets);
}
...
void AllSockets::Remove (Socket* _pSocket)
{
	...
	pthread_mutex_lock (&mtx_AllSockets);
	dq_InternalSocketManagementRequest.push_back (InternalSocketRequest (REMOVE_SOCKET, _pSocket));
	pthread_mutex_unlock (&mtx_AllSockets);
}
...
void AllSockets::CloseAndDeleteAllSockets (void)
{
	...
	pthread_mutex_lock (&mtx_AllSockets);
	dq_InternalSocketManagementRequest.push_back (InternalSocketRequest (CLEAR_SOCKETS, NULL));
	pthread_mutex_unlock (&mtx_AllSockets);
}
...
void AllSockets::ManageSocket (int _iSocketFd, SocketEventCallback_fn_t _fnCallback)
{
	...
	dq_ExternalSocketManagementRequest.push_back (SocketManagementRequest (ADD_MANAGED_SOCKET,
																   _iSocketFd,
																   _fnCallback));
...
{
	pthread_mutex_lock (&mtx_ExternalSocketManagement);
	dq_ExternalSocketManagementRequest.push_back (SocketManagementRequest (REMOVE_MANAGED_SOCKET,
																   _iSocketFd));
	pthread_mutex_unlock (&mtx_ExternalSocketManagement);
}
...
{
	pthread_mutex_lock (&mtx_ExternalSocketManagement);
	dq_ExternalSocketManagementRequest.push_back (SocketManagementRequest (CLEAR_MANAGED_SOCKETS));
	pthread_mutex_unlock (&mtx_ExternalSocketManagement);
}
...
{
	pthread_mutex_lock (&mtx_AllSockets);
	ITERATE (InternalRequest_dq_t, dq_InternalSocketManagementRequest, ite)
	{
		if (CLEAR_SOCKETS == ite->c_Type)
		{
			ITERATE (Socket_set_t, set_AllSockets, ite)
			 	(*ite)->Close ();
			set_AllSockets.clear ();

			dq_InternalSocketManagementRequest.clear ();
			pthread_mutex_unlock (&mtx_AllSockets);
			return;
		}
		else if (REMOVE_SOCKET == ite->c_Type)
			set_AllSockets.erase (ite->p_Socket);
		else if (ADD_SOCKET == ite->c_Type)
			set_AllSockets.insert (ite->p_Socket);
	}
	dq_InternalSocketManagementRequest.clear ();
	pthread_mutex_unlock (&mtx_AllSockets);
...
void AllSockets::ProcessInternalSockets (fd_set& _fsRead, fd_set& _fsWrite, fd_set& _fsError)
{
	...
	ITERATE (Socket_set_t, set_AllSockets, ite)
	{
		...
		if (false == IsValidSocket (pSocket))
			continue;
		if ((SOCKET_FD_UNDEF == pSocket->fd_Socket) ||
			(SOCKET_FD_CLOSED == pSocket->fd_Socket) ||
			(SOCKET_FD_DESTROYED == pSocket->fd_Socket))
...
void AllSockets::ProcessExternalSockets (fd_set& _fsRead, fd_set& _fsWrite, fd_set& _fsError)
{
	...
	pthread_mutex_lock (&mtx_ExternalSocketManagement);
	ITERATE (ManagementRequest_dq_t, dq_ExternalSocketManagementRequest, ite)
	{
	...
	dq_ExternalSocketManagementRequest.clear ();
	pthread_mutex_unlock (&mtx_ExternalSocketManagement);
