									...
									double_Vec_t& _rvecTrace)
{
	...
	for (int i = 0; i < iSize; ++ i)
	{
		...
		// This is to resolve the out-of-control weight	
		// growth issue.  Not sure of exact cause...	
		/*
		if (d_MaxWeightCeiling > 0)
		{
			if (vec_Weights [i] > d_MaxWeightCeiling)
				vec_Weights [i] = d_MaxWeightCeiling;
			else if (vec_Weights [i] < - d_MaxWeightCeiling)
				vec_Weights [i] = - d_MaxWeightCeiling;
		}
		*/
