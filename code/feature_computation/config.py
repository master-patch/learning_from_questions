# USAGE															
# - from config import *
# - call load_config (sys.argv) first
# - call get_config ('config field name') to get configuration values
# - command line: <program> [config file name] [field = value]*
...
# 
def get_bool(_sConfig):
	return bool(int(_get_config(_sConfig)));

def get_string(_sConfig):
	return _get_config(_sConfig);

def get_int(_sConfig):
	return int(_get_config(_sConfig));

def get_float(_sConfig):
	return float(_get_config(_sConfig));
