(define (domain minecraft-beta)
	...
	(:functions
		...
		(duration-need  ?tool - tool  ?res - thing)
	...
	(:action get-harvest-wheat
		...
		:effect
			(and
				...
				(increase (thing-available seeds) 2)
...
;	(:action xc
;		:parameters (?loc1 - mapgrid  ?loc2 - mapgrid)
;		:precondition (connect ?loc1 ?loc2)
;		:effect (not (connect ?loc1 ?loc2))
;	)
