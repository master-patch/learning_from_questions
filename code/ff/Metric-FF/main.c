...
{
	struct rlimit lim;
	if (-1 == getrlimit (RLIMIT_STACK, &lim))
		printf ("getrlimit returned error!\n");
	else
	{
		/*printf ("stack limits: %ld soft, %ld hard.\n",
				lim.rlim_cur, lim.rlim_max);
		if (RLIM_INFINITY == lim.rlim_max)
			printf ("hard limit is inf.\n");*/
	}
	/* printf ("increasing stack soft limit...\n");*/
	lim.rlim_cur *= 10;
	...
	*/
	...
	if ( found_plan )
	{
		print_plan();
		/* printf ("-----END-STATE-----\n");
		print_State (gplan_states [gnum_plan_ops]);
		printf ("-----END-STATE-----\n");*/
	printf("--------------------------\n");
